...
class App extends Component {
  ...
  previousClick() {
    ...
    else index = this.state.currentIndex - 1;
    this.setState({currentIndex: index});
    this.setState({catalogSelected: catalogs[index]});
  }
  ...
  nextClick() {
    let index
    if(this.state.currentIndex === catalogs.length - 1)
     index = 0;
    else index = this.state.currentIndex + 1;
    this.setState({currentIndex: index});
    this.setState({catalogSelected: catalogs[index]});
  ...
  slideChange(event) {
    if(event.target.checked ){
      ...
      interval = setInterval(this.nextClick, this.state.slideDuration);
    }
    else {
